...
RUN apt update && apt install -y exiftool && apt-get install -y inotify-tools && apt install -y libmagickwand-dev  && apt install -y wget && \
wget http://do-prd-mvn-01.do.viaa.be:8081/repository/debpkg/iiif/kakadu_8.4-1_amd64.deb && dpkg -i kakadu_8.4-1_amd64.deb && rm kakadu_8.4-1_amd64.deb && rm -rf /var/lib/apt/lists/*
